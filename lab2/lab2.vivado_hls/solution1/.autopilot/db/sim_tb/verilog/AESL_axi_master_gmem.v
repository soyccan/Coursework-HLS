...
module AESL_axi_master_gmem (
    ...
    TRAN_gmem_BID,
    TRAN_gmem_BUSER,
    TRAN_gmem_pn32HPInput,
    TRAN_gmem_pn32HPOutput,
    ready,
    done
    );

//------------------------Parameter----------------------
`define TV_IN_gmem "./c.fir_n11_maxi.autotvin_gmem.dat"
`define TV_OUT_gmem "./rtl.fir_n11_maxi.autotvout_gmem.dat"
...
output [32 - 1 : 0] TRAN_gmem_pn32HPInput;
output [32 - 1 : 0] TRAN_gmem_pn32HPOutput;
input ready;
input done;
...
reg RLAST_tmp = 0;
reg RVALID_tmp = 0;
reg [32 - 1 : 0] pn32HPInput = 0;
reg [32 - 1 : 0] pn32HPOutput = 0;
...
assign    TRAN_gmem_pn32HPInput = pn32HPInput;
assign    TRAN_gmem_pn32HPOutput = pn32HPOutput;
...
initial begin : initialize_offset
  ...
  pn32HPInput <= 0 * DATA_byte_num;
  pn32HPOutput <= 600 * DATA_byte_num;
end
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("D:/Vivado/lab2_hls/solution1/sim/verilog/AESL_axi_master_gmem.v: Write request address %d exceed AXI master gmem array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("D:/Vivado/lab2_hls/solution1/sim/verilog/AESL_axi_master_gmem.v: Read request address %d exceed AXI master gmem array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
