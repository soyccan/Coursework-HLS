...
typedef struct {
    ...
    XMultip_2num_uio_map maps[ MAX_UIO_MAPS ];
} XMultip_2num_uio_info;

/***************** Variable Definitions **************************************/
static XMultip_2num_uio_info uio_info;
...
static int uio_info_read_name(XMultip_2num_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMultip_2num_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMultip_2num_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMultip_2num_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMultip_2num_Initialize(XMultip_2num *InstancePtr, const char* InstanceName) {
	XMultip_2num_uio_info *InfoPtr = &uio_info;
